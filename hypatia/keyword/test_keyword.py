...
class _TestCaseBase:
    ...
    def test_index_doc_same_value(self):
        ...
        assert sorted(index._fwd_index[1]) == [1]
        index.index_doc(1, [1, 2])
        assert sorted(index._fwd_index[1]) == [1]
    ...
    def test_reindex_doc_same_values(self):
        ...
        assert 1 in index._fwd_index[1]
        assert 1 in index._fwd_index[2]
        assert 1 in index._fwd_index[3]
        assert not (4 in index._fwd_index)
    ...
    def test_reindex_doc_different_values(self):
        ...
        assert index.indexed_count() == 1
        assert 1 in index._rev_index
        assert not (1 in index._fwd_index.get(1, []))
        assert 1 in index._fwd_index[2]
        assert 1 in index._fwd_index[3]
        assert 1 in index._fwd_index[4]
    ...
    def test_hasdoc(self):
        index = self._makeOne()
        self._populate(index)
        assert index.has_doc(1) == 1
        assert index.has_doc(2) == 1
        assert index.has_doc(3) == 1
        assert index.has_doc(4) == 0
        assert index.has_doc(5) == 1
        assert index.has_doc(6) == 0

    def test_unique_values(self):
        index = self._makeOne()
        self._populate(index)
        assert list(index.unique_values()) == [
            "CMF",
            "FOX",
            "Zope",
            "Zope3",
    ...
    def test_all(self):
        ...
        index = self._makeOne()
        result = index.all([1])
        assert result.__class__ == query.All
        assert result._value == [1]
    ...
    def test_notall(self):
        ...
        assert result.__class__ == query.NotAll
        assert result._value == [1]
...
class TestsKeywordIndex32(
    _KeywordIndexTestsBase,
    _ThirtyTwoBitBase,
    _TestCaseBase,
):
    pass
