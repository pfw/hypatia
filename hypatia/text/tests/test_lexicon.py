...
class LexiconTests(unittest.TestCase):
    ...
    def test_parseTerms_empty_string(self):
        lexicon = self._makeOne()
        self.assertEqual(lexicon.parseTerms("a b*c de?f"), ["a", "b*c", "de?f"])
...
class SplitterTests(unittest.TestCase):
    ...
    def test_process_w_locale_awareness(self):
        ...
        try:
            ...
            expected = ["m\xfclltonne", "waschb\xe4r", "beh\xf6rde", "\xfcberflieger"]
            splitter = self._makeOne()
    ...
    def test_process_w_glob(self):
        splitter = self._makeOne()
        self.assertEqual(
            splitter.process(["abc?def hij*klm nop* qrs?"]),
    ...
    def test_processGlob_w_glob(self):
        splitter = self._makeOne()
        self.assertEqual(
            splitter.processGlob(["abc?def hij*klm nop* qrs?"]),
            ["abc?def", "hij*klm", "nop*", "qrs?"],
        )
...
class StopWordRemoverTests(unittest.TestCase):
    ...
    def test_process_nonempty(self):
        ...
        cn = self._makeOne()
        self.assertEqual(
            cn.process(QUOTE.lower().split()), ["end", "government", "justice"]
        )
