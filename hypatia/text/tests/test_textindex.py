...
class TextIndexTests(unittest.TestCase):
    ...
    def _makeOne(
        self, discriminator=_marker, lexicon=_marker, index=_marker, family=None
    ):
        ...
        if lexicon is _marker:
            if index is _marker:  # defaults
                return self._getTargetClass()(
                    discriminator=discriminator, family=family
                )
            else:
                return self._getTargetClass()(
                    discriminator=discriminator, index=index, family=family
                )
        else:
            if index is _marker:
                return self._getTargetClass()(
                    discriminator=discriminator, lexicon=lexicon, family=family
                )
            else:
                return self._getTargetClass()(
    ...
    def test_document_repr(self):
        ...
        self.assertEqual(index.document_repr(1), "simple document contains five words")
        self.assertEqual(index.document_repr(50), None)
    ...
    def test_sort_with_extra_kwargs(self):
        ...
        expect = [-2, 0]
        self.assertEqual(
            index.sort(results, limit=2, sort_type="bar", raise_unsortable=False),
            expect,
        )
