...
class TestQueryParserBase(unittest.TestCase):
    ...
    def _compareParseTrees(self, got, expected, msg=None):
        ...
        if isinstance(got, PhraseNode):
            ...
            self.assertEqual(
                got.nodeType(), isinstance(got, AndNode) and "AND" or "OR", msg
            )
            list1 = got.getValue()
...
class TestQueryParser(TestQueryParserBase):
    ...
    def test007(self):
        ...
        parser = self._makeOne()
        self._expect(
            parser, "aa AND NOT bb", AndNode([AtomNode("aa"), NotNode(AtomNode("bb"))])
        )
    ...
    def test008(self):
        ...
        parser = self._makeOne()
        self._expect(
            parser, "aa NOT bb", AndNode([AtomNode("aa"), NotNode(AtomNode("bb"))])
        )
    ...
    def test015(self):
        ...
        parser = self._makeOne()
        self._expect(
            parser, "foo -bar", AndNode([AtomNode("foo"), NotNode(AtomNode("bar"))])
        )
    ...
    def test016(self):
        ...
        parser = self._makeOne()
        self._expect(
            parser, "-foo bar", AndNode([AtomNode("bar"), NotNode(AtomNode("foo"))])
        )
    ...
    def test017(self):
        ...
        self._expect(
            parser,
            "booh -foo-bar",
            AndNode([AtomNode("booh"), NotNode(PhraseNode(["foo", "bar"]))]),
        )
    ...
    def test021(self):
        ...
        self._expect(
            parser,
            'foo"bar"blech',
            AndNode([AtomNode("foo"), AtomNode("bar"), AtomNode("blech")]),
        )
    ...
    def test023(self):
        ...
        parser = self._makeOne()
        self._expect(parser, "foo* bar", AndNode([GlobNode("foo*"), AtomNode("bar")]))
